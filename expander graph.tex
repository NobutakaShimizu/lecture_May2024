...
\begin{proposition}{低直径性}{small diameter}
    $n$頂点$(1-\gamma)$-エクスパンダーの直径は高々$100\log n$.
\end{proposition}
\begin{proof}
    二頂点$u,v$を任意に固定し,
    $u$を始点とした単純ランダムウォーク$(X_t)_{t\ge 0}$を考える.
    定常分布$\pi$は$\pimin \ge \frac{1}{n^2}$を満たす.
    従って\cref{lem:mixing time and spectral gap}を$\varepsilon=n^{-10}$に対して適用すると,
    \[
        \tmix(n^{-10}) \le \frac{1}{\gamma}\log \frac{1}{2n^{12}} \le \frac{13\log n}{\gamma}.
    \]
    $\ell=\ceil*{\frac{1}{\gamma}\log \frac{1}{2n^{12}} \le \frac{13\log n}{\gamma}}$とする.
    混交時間の定義(\cref{def:mixing time})より$\dtv(X_{\ell}, \pi) \le n^{-10}$であり,
    $\pi(v) \ge n^{-2}$なので,
    \[
        \Pr[X_\ell = v] \ge \pi(v) - \dtv(X_\ell,\pi) >0
    \]
    すなわち, 正の確率で$X_\ell = v$となるので, $\dist(u,v) \le \ell$となる.
\end{proof}
