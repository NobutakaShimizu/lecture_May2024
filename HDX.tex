...
\begin{definition}{下降ウォークと面上の定常分布}{down walk and stationary distribution}
    ...
    各$i=0,\dots,d-1$に対して, $X(i)$上の分布$\pi_i \in [0,1]^{X(i)}$を
        \begin{itemize}
        \item $i=d-1$のとき: $X$が$\pi \in [0,1]^{X(d-1)}$で重み付けられた単体複体ならば$\pi_{d-1} = \pi$とする. そうでないならば$\pi_{d-1}$は$X(d-1)$上の一様分布. すなわち$\pi_{d-1}(\sigma) = \frac{1}{\abs{X(d-1)}}$.
...
\begin{proof}
    ...
    より主張を得る.
\end{proof}
\begin{lemma}{成分}{PUD seibun}
    上昇下降ウォーク$\PUD_i$の各成分は
        \[
            \PUD_i(\sigma,\sigma') = \begin{cases}
                \frac{\pi_{i+1}(\sigma\cup\sigma')}{(i+1)(i+2)\pi_i(\sigma)}
...
\begin{lemma}{}{local RW seibun}
    ...
    \begin{align*}
        ...
        \end{cases} \\
        &= \begin{cases}
            (i+2)\PUD_{i+1}\rbra*{\sigma\cup\{u\}, \sigma\cup\{v\}}	& \tif \{u,v\}\in E_\sigma,\\
            0 & \totherwise.
        \end{cases}
    \end{align*}
\end{lemma}
...
すなわち, $f_\sigma\colon X_\sigma(0) \to \Real$を
\[
...
以下の補題は$\ispace$上の内積が各リンク$X_\sigma$上の内積の線形和に分解できることを表す.

\begin{lemma}{}{nonlazyUP decomposition}
    純粋な$d$次元単体複体$X$と任意の$0 \le i\le d,f \in \ispace$に対し
    \[ \iprod{f, \nonlazyPUD_i f} = \E_{\sigma\sim \pi_{i-1}}\sbra*{\abra*{ f_\sigma, P_\sigma f_\sigma }_{\pi^\sigma_0}}. \]
    ここで, $\pi^\sigma_0 \in [0,1]^{X_\sigma(0)}$はリンク$X_\sigma$の頂点集合$X_\sigma(0)$上の定常分布である.
\end{lemma}
\begin{proof}
...
\end{proof}
