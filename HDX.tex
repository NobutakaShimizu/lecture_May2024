...
\begin{definition}{上昇ウォーク}{up walk}
    ...
    \begin{align*}
        ...
        \end{cases}
    \end{align*}
    で定める\footnote{全ての$\sigma\in X(i)$に対し$\pi_i(\sigma)>00$である
...
\begin{remark}{「上昇」「下降」の名称}{up and down}
    ...
    内積空間を考え随伴性などを考える場合(\cref{sec:inaiseki})は左から作用させる.
...
従って, $e_1,e_2\in E\setminus F$が森であることの必要十分条件は$e_1,e_2$が同じ連結成分間を繋がない\footnote{すなわち, 次の命題の否定である: ある$i \neq j\in\{1,\dots,\ell\}$に対し$e_1,e_2$は$C_i,C_j$間の辺でない}ことである.



\section{単体複体の局所大域原理}
\citet{KO20}は単体複体には局所大域原理,
すなわち, 局所エクスパンダー性は大域エクスパンダー性を導くことを証明した.
...
\end{theorem}
この結果は\citet{KO20}によって2018年の国際会議RANDOMにて発表され, 2020年の国際会議STOCにて\citet{AL20}によって以下のように改善された.
%
...
本質的には, 高次元エクスパンダーがもつ局所大域原理, すなわち
\emph{大域的なランダムウォーク(上昇下降ランダムウォーク)の混交性が局所的なランダムウォークの混交性に帰着される}という性質が極めて重要であり,
