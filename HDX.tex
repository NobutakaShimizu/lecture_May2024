...
\begin{proof}
    後半の等号は
    \begin{align*}
        \abra*{ f_\rho, J_\rho g }_{X_\rho(0)}
        &= \abra*{ f_\rho, J_\rho g }_{X_\rho(0)} \\
        &= \E_{\substack{u\sim \pi^\rho_0 \\ v\sim J_\rho(u,\cdot)}}\sbra*{ f_\rho(u)g_\rho(v) } \\
        &= \E_{\substack{u\sim \pi^\rho_0 \\ v\sim \pi^\rho_0}}\sbra*{ f_\rho(u)g_\rho(v) } \\
        &= \E_{\pi^\rho_0}[f_\rho]\cdot \E_{\pi^\rho_0}[g_\rho]
    \end{align*}
    から従うので, 前半の等号を示せばよい.
    前半の等号は, 上昇ウォークと下降ウォークの随伴性\cref{eq:adjoint up and walk}より以下のようにして得られる:
    \begin{align*}
...
\cref{thm:Kaufman-Oppenheim theorem}は\citet{AL20}によって改善され以下の結果が示されている:
%
