...
非周期性は収束性を保証する性質である.
ここでは\cref{thm:random walk convergence}の最後の主張(既約的かつ非周期的なランダムウォークの一意収束性)の証明を与える.
まず, 以下の三つの補題を証明する.
...
\begin{proof}
  ...
  $\gcd(a,b,c)=1$より,
  任意の$M\in\Nat$に対し
  ある整数$x,y,z\in \Int$が存在して$ax+by+cz=1$を満たす.
  このような$x,y,z$は一意ではなく,
...
\begin{proof}
  任意の$u\in V$に対し
  \[
    \abs{xP(u)} \le \sum_{v\in V}\abs{x(v)}P(u,v) \le \norm{x}_{\infty}\cdot \sum_{v\in V}P(u,v) = \norm{x}_{\infty}.
  \]
\end{proof}
\begin{proof}[\textbf{\cref{thm:random walk convergence}の証明.}]
  ...
  従って, ある$\delta>0$, 全成分が$1$の行列$J$, 確率行列$R$を用いて
  \[
    Q = \delta J + (1-\delta)R
  \]
  と表せる.
  従って, $\allone$に直交する任意のベクトルに対し$xQ^t = \sum_{i=0}^t \delta^i(1-\delta)^{t-i} x J^iR^{t-i} = (1-\delta)^t x R^t$が成り立つ.
  初期分布$p_0\in[0,1]^V$に対し, $x\defeq p_0 - \pi$とすると
  \begin{align*}
    \norm{p_t - \pi}_{\infty} &= \norm{(p_0-\pi)P^t}_{\infty} \\
    &\le \norm{xQ^{\floor{t/L}}}_{\infty} & & \text{$\because$\cref{lem:infinity norm}}\\
    &= (1-\delta)^t\norm{x R^{\floor{t/L}}}_\infty \\
    & \le (1-\delta)^{\floor{t/L}} \norm{x}_{\infty} & & \text{$\because$\cref{lem:infinity norm}} \\
    &\le (1-\delta)^{\floor{t/L}} \to 0
  \end{align*}
  を得る.
