...
ランダムウォークの分布$p_t$を考える場合, 遷移確率行列$P$を右から作用させたものを考える.
\end{comment}
...
\begin{definition}{グラフ}{graph}
  ...
  二頂点$u,v\in V$が$\{u,v\}\in E$を満たすとき, $u$は$v$に\emph{隣接 (adjacent)}しているという ($v$もまた$u$に隣接している).
...
\end{definition}
グラフの路と閉路を定義する.
\begin{definition}{路と閉路}{path and cycle}
  グラフ$G=(V,E)$を考える.
