...
%
\begin{definition}{重み付きグラフ}{weighted graph}
    グラフ$G=(V,E)$と関数$W\colon E \to \Real_{>0}$に対し, 三つ組$(V,E,W)$
    を\emph{重み付きグラフ (weighted graph)}という.
    頂点$u\in V$の\emph{重み付き次数 (weighted degree)}を$\deg_W(u)=\sum_{w\in V,\{u,w\}\in E} W(\{u,w\})$で定める.

    辺重みグラフ$(V,E,W)$に対し, 遷移確率行列$P$が
        \[
            P(u,v) = \begin{cases}
                \frac{w(\{u,v\})}{\deg_W(u)}	& \text{if }uv\in E,\\
                0 & \text{otherwise}
            \end{cases}
        \]
    で与えられるランダムウォークを\emph{重み付き単純ランダムウォーク (weighted simple random walk)}と呼ぶ.
\end{definition}

重み関数$W$を, 全ての$e\in E$に対し$W(e)=1$とすると通常の単純ランダムウォーク(\cref{def:SRW})と同じ遷移確率行列になる.
